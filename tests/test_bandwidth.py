...
reproduces the samples it was built from, and that score is monotone in the bandwidth. Its minimum
is at zero, where the smoother hands every sample its own value back and says nothing whatever
about the space between them -- which is the only place anybody ever asks about.
...
(The application this came from is correcting an all-sky camera's plate, where the samples are
stars and the place being asked about is wherever the meteor was.)
"""
import numpy as np
import pytest
from demetria.metrics import euclidean

from demetria.correctors import kernels
from demetria.correctors.bandwidth import DEFAULT_MIN, MIN_POINTS, loo_score, select
...
@pytest.fixture
def sample():
    """ 120 points on the disk, the field sampled at them, and noise a tenth of its amplitude. """
